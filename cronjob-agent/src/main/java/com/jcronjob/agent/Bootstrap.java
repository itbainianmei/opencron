...
public class Bootstrap implements Serializable {
    ...
    public void init() throws Exception {
        port = Integer.valueOf(Integer.parseInt(Globals.CRONJOB_PORT));
        String inputPwd = Globals.CRONJOB_PASSWORD;
        if (notEmpty(inputPwd)) {
            this.password = DigestUtils.md5Hex(inputPwd).toLowerCase();
            Globals.CRONJOB_PASSWORD_FILE.deleteOnExit();
            IOUtils.writeText(Globals.CRONJOB_PASSWORD_FILE, this.password, CHARSET);
        } else {
            if (!Globals.CRONJOB_PASSWORD_FILE.exists()) {
                this.password = DigestUtils.md5Hex(this.password).toLowerCase();
                IOUtils.writeText(Globals.CRONJOB_PASSWORD_FILE, this.password, CHARSET);
            } else {
                password = IOUtils.readText(Globals.CRONJOB_PASSWORD_FILE, CHARSET).trim().toLowerCase();
            }
        }
    }
    ...
    private void await() throws Exception {
        // Negative values - don't wait on port - cronjob is embedded or we just don't like ports
        if (port == -2) {
            return;
        }
        if (port == -1) {
        ...
        try {
            awaitSocket = new ServerSocket(CronjobProperties.getInt("cronjob.shutdown"));
        } catch (IOException e) {
            logger.error("[cronjob] agent .await: create[{}] ", CronjobProperties.getInt("cronjob.shutdown"), e);
            return;
        }
            ...
            while (!stopAwait) {
                ...
                try {
                    ...
                    try {
                        ...
                        // it does.
                        logger.warn("[cronjob] agentServer accept.timeout", Long.valueOf(System.currentTimeMillis() - acceptStartTime), ste);
                        continue;
                    } catch (AccessControlException ace) {
                        logger.warn("[cronjob] agentServer .accept security exception: {}", ace.getMessage(), ace);
                        continue;
                    } catch (IOException e) {
                        if (stopAwait) {
                            break;
                        }
                        logger.error("[cronjob] agent .await: accept: ", e);
                        break;
                    }
                    ...
                    while (expected > 0) {
                        ...
                        try {
                            ...
                            logger.warn("[cronjob] agent .await: read: ", e);
                            ch = -1;
                        }
                ...
                if (match) {
                    ...
                    logger.warn("[cronjob] agent .await: Invalid command '" + command.toString() + "' received");
                }
            }
        } finally {
            ServerSocket serverSocket = awaitSocket;
    ...
    private void shutdown() throws Exception {
         ...
         */
        Socket socket = new Socket("localhost",CronjobProperties.getInt("cronjob.shutdown"));
        ...
        while (!((reply = br.readLine()) == null)) {
            logger.info("[cronjob]shutdown:{}" + reply);
        }
        br.close();
        is.close();
        pw.close();
    ...
    private void stopServer() {
        if (this.server != null && this.server.isServing()) {
             ...
             * delete pid file...
             */
            Globals.CRONJOB_PID_FILE.deleteOnExit();
            System.exit(0);
        }
    }
